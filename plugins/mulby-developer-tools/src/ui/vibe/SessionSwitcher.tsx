...
import { ChevronDown, Plus, Trash2, HardDrive, MessageSquarePlus } from 'lucide-react'
...
export function SessionSwitcher({ onNewSession, onNewConversation }: Props) {
  ...
  return (
    <div className="relative" ref={dropRef}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors text-left"
      >
        {activeSession ? (
          <>
            <span className={`w-2 h-2 rounded-full shrink-0 ${STATE_DOT[activeSession.state]}`} />
            <span className="truncate text-xs font-medium text-slate-700 dark:text-slate-200 flex-1 min-w-0">
          ...
          <span className="text-xs text-slate-400 flex-1 truncate">未选择项目</span>
        )}
        <ChevronDown size={12} className={`shrink-0 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="max-h-64 overflow-auto">
            {sessions.length === 0 ? (
              <div className="px-3 py-4 text-center text-xs text-slate-400">暂无项目</div>
            ) : (
              sessions.map((s) => (
                <SessionItem
                  key={s.id}
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 flex items-center">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 text-xs text-emerald-600 dark:text-emerald-400 hover:bg-slate-50 dark:hover:bg-slate-800/50"
              title="新建一个全新的插件项目"
            >
              <Plus size={13} /> 新建项目
            </button>
            {activeSession && onNewConversation && (
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 text-xs text-sky-600 dark:text-sky-400 border-l border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50"
            ...
            <button
              ...
              className="px-3 py-2 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 border-l border-slate-200 dark:border-slate-700"
              title="存储占用"
            >
              <HardDrive size={13} />
            </button>
...
function SessionItem({ session, active, onSelect, onDelete }: { session: VibeSession; active: boolean; onSelect: () => void; onDelete: () => void }) {
  ...
  const convLabel = firstUser ? (firstUser.length > 16 ? `${firstUser.slice(0, 16)}…` : firstUser) : '新对话'
  return (
    <div
      onClick={onSelect}
    ...
    >
      <span className={`w-2 h-2 rounded-full shrink-0 ${STATE_DOT[session.state]}`} title={STATE_LABEL[session.state]} />
      <div className="flex-1 min-w-0">
        <div className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate">{session.pluginName}</div>
        <div className="text-[10px] text-slate-400 dark:text-slate-500 truncate">
          {convLabel} · {relativeTime(session.lastActiveAt)}
        </div>
      </div>
      {!active && (
        <button
