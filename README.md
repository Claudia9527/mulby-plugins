...
Mulby 插件集合仓库，开放社区共创。每个插件位于 `plugins/<plugin-name>/`，支持统一构建、打包，并通过 GitHub Release 分发 `.inplugin` 产物。

**想要贡献插件？** 👉 查看 [贡献指南（CONTRIBUTING.md）](./CONTRIBUTING.md)
...
- **增量发布**：CI 自动检测变更的插件，只构建和发布有改动的部分。
- **PR 门禁**：社区 PR 会自动校验插件结构、manifest 和构建。
...
- 本仓库使用 **pnpm workspace**（根目录见 `pnpm-workspace.yaml` 与 `pnpm-lock.yaml`）。请先安装 pnpm（建议 `corepack enable` 后使用 `package.json` 中的 `packageManager` 版本）。
- 打包前需先构建：通常先执行 `pnpm run build`，再执行 `pnpm run pack` 产出 `.inplugin`。
- 在 AI 代写流程中不建议使用 `mulby create --ai`。

## AI 辅助开发

推荐使用 [mulby-skills](https://github.com/Unicellular-SU/mulby-skills) 中的 **develop-mulby-plugin** skill，在 Cursor、Claude 等 AI IDE 中辅助开发 Mulby 插件。该 skill 涵盖脚手架创建、manifest 设计、API 选用、前端应用转换、旧生态迁移、图标生成等完整开发指导。
...
1. 从 `dev` 拉出功能分支：`feature/<plugin-or-topic>`
2. 功能完成后合并回 `dev` 做集成验证。
3. 准备发布时，将 `dev` 合并到 `main`。
4. `push main` 后由 GitHub Actions 自动检测变更、增量构建并发布 Release。

### 社区贡献者

1. Fork 本仓库到自己的 GitHub 账号。
2. 在 Fork 中创建功能分支开发插件。
3. 向 `main`（或 `dev`）提交 Pull Request。
4. CI 自动校验 + 维护者审核 → 合并后自动发布。
...
- 作用：在仓库根执行 `pnpm install`，再遍历 `plugins/` 下有 `build` 脚本的插件执行 `pnpm run build`。
- 特点：只做“构建”，不打包、不生成 `plugins.json`。
- 用途：快速检查多个插件是否可编译通过。
- 命令：`pnpm run build-all`

### `scripts/screenshot-plugin-ui.js`

- 作用：为有 UI 的插件自动构建并截取首屏图，输出到 `plugins/<name>/screenshots/1.png`，同时补充 `1.txt` caption。
- 依赖：根目录安装 `puppeteer`（`pnpm install`）。
- 用途：用于补齐插件商店截图资源。
- 命令：
  - `pnpm run screenshot-plugins`（全部）
  - `pnpm run screenshot-plugins -- <plugin-name>`（指定插件）
...
| 工作流 | 触发条件 | 功能 |
|--------|----------|------|
| `pr-check.yml` | Pull Request → main/dev | 自动校验插件结构、manifest、构建，在 PR 评论区生成报告 |
| `build.yml` | Push → main | 增量检测变更插件，构建、打包、发布到 Release，更新 `plugins.json` |
